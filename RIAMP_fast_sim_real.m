...
people_indices = randsample(Ntotal, 2*N);
people_indices_tr = people_indices(1:N);
people_indices_te = people_indices((N+1):2*N);
genomat_filt = genomat(people_indices_tr, 1:M);
genomat_filt_te = genomat(people_indices_te, 1:M);
I = find(genomat_filt == -1);
genomat_filt(I) = 0;
I_te = find(genomat_filt_te == -1);
genomat_filt_te(I_te) = 0;

X = normalize( double(genomat_filt) );
X = X / sqrt(N);
X_te = normalize( double(genomat_filt_te) );
X_te = X_te / sqrt(N);


%simulating under the model   
distr = struct('eta', [0 0.001, 0.1 1 10]', 'probs', [0.85 0.05 0.05 0.03 0.02]');
distr = struct('eta', [0 1]', 'probs', [0.9 0.1]');
distr = struct('eta', N * [0 1.131e-8 4.81e-5]', 'probs', [0.711 0.2644 0.0246]');
corr0 = sqrt(0.0);

%setting gamw such that SNR = 1
SNR = 1;
gamw = SNR * N / (M * distr.eta' * distr.probs);
