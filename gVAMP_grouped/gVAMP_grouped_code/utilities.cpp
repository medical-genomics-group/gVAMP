...
void initialize_prior(std::vector<std::vector<double>>& probs,  std::vector<std::vector<double>>& vars, int N, int Mt, int rank){

    if (probs.size()!=0 || vars.size()!=0) 
        return;
    else
        throw std::invalid_argument("No probabilities or variances were specified");
    /*
    int num_mix = 23;
    ...
    for (int i = 0; i , probs.size();, i++){
        probs[i].push_back(1 - 50000.0/Mt);
    }
