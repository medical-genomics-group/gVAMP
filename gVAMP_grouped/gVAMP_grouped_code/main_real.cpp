...
int main(int argc, char** argv)
{
    ...
    if (opt.get_run_mode() == "infere"){ 
        ...
        data dataset(phenfp, opt.get_bed_file(), opt.get_group_file(), opt.get_N(), M, opt.get_Mt(), opt.get_K(), S, rank, type_data, alpha_scale, bimfp);
        // dataset.read_phen();
	...
	if (!opt.get_true_signal_files().empty()) {
   	 // Get first true signal file (assuming only one file is provided)
    	std::string true_signal_file = opt.get_true_signal_files()[0];
   	if (rank == 0)
        std::cout << "Reading true signal from: " << true_signal_file << std::endl;
        
   	 // Read the file using existing read_vec_from_file utility
   	 beta_true = read_vec_from_file(true_signal_file, M, S);
	} else {
    	// Default to zero vector if no file provided
    ...
    else if (opt.get_run_mode() == "both")
    {
        ...
        double alpha_scale = opt.get_alpha_scale();
        std::string bimfp = opt.get_bim_file();
        data dataset(phenfp, opt.get_bed_file(), opt.get_group_file(), opt.get_N(), M, opt.get_Mt(), K,  S, rank, type_data, alpha_scale, bimfp);
        // dataset.read_phen();
        // dataset.read_genotype_data();
        // dataset.compute_markers_statistics();
        ...
        data dataset_test(pheno_test, bedfp_test, groupfp_test, N_test, M_test, Mt_test, K_test, S, rank, type_data, alpha_scale, bimfp);
        // dataset_test.read_phen();
        // dataset_test.read_genotype_data();
        // dataset_test.compute_markers_statistics();
    ...
    data dataset_test(phen, bedfp_test, groupfp_test, N_test, M_test, Mt_test, K, S_test, rank, type_data, alpha_scale, bimfp);
    ...
    data dataset_test(y, bedfp_test, groupfp_test, N_test, M_test, Mt_test, K, S_test, rank, type_data, alpha_scale, bimfp);
