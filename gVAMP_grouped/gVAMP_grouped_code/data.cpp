...
//
data::data(std::string fp, std::string genofp, std::string groupfp, const int N, const int M, const int Mt, int K, const int S, const int rank, std::string type_data, double alpha_scale, std::string bimfp) :
    phenfp(fp),
    bimfp(bimfp),
    groupfp(groupfp),
    type_data(type_data),
    ...
    if (!groupfp.empty()) {
        ...
        // Default: all markers in group 0
        group_assignments = std::vector<int>(M, 0);
        K = 1;
    }
...
//
data::data(std::vector<double> y, std::string genofp, std::string groupfp, const int N, const int M, const int Mt, int K, const int S, const int rank, std::string type_data, double alpha_scale, std::string bimfp) :
    type_data(type_data),
    bimfp(bimfp),
    N(N),
    M(M),
...
void data::read_phen() {
    ...
    if (infile.is_open()) {
        int line_n = 0;
        nonas = 0, nas = 0;
	//std::getline(infile, line); // KK: skipping header file, appea
...
void data::read_group_assignments() {
    if (groupfp.empty()) {
        // If no group file provided, assign all to group 0
        group_assignments = std::vector<int>(M, 0);  // Changed Mt to M since we only need local assignments
        K = 1;
        return;
    }

    // Open file
    std::ifstream inFile(groupfp);
    if (!inFile.is_open()) {
        std::cout << "FATAL: could not open group file " << groupfp << std::endl;
        exit(EXIT_FAILURE);
    }

    // Count total lines first
    size_t total_lines = std::count(std::istreambuf_iterator<char>(inFile),
                                  std::istreambuf_iterator<char>(), '\n');
    if (total_lines != Mt) {
        std::cout << "FATAL: number of group assignments (" << total_lines 
                  << ") does not match Mt (" << Mt << ")" << std::endl;
        exit(EXIT_FAILURE);
    }
