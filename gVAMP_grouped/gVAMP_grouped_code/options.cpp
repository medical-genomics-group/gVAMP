...
void Options::read_command_line_options(int argc, char** argv) {
    ...
    for (int i=1; i<argc; ++i) {
        ...
        */
        // List of phenotype files to read; comma separated if more than one.
        ...
        else if (!strcmp(argv[i], "--group-file")) {
            if (i == argc - 1) fail_if_last(argv, i);
            group_file = argv[++i];
            ss << "--group-file " << group_file << "\n";
            
            // Read and validate group assignments
            std::ifstream inFile(group_file);
            if (!inFile.is_open()) {
            ...
            // Split into groups first, groups are separated by semicolon
            std::stringstream group_list(cslist);
            std::string group_values;
            vars = std::vector<std::vector<double>>(K); // Initialize with K empty vectors
            int group_idx = 0;
            
            while (getline(group_list, group_values, '@')) {
                if (group_idx >= K) {
                    std::cout << "FATAL: too many groups in vars list (expected " << K << ")" << std::endl;
                    exit(EXIT_FAILURE);
                }
                ...
                while (getline(value_list, value, ',')) {
                    vars[group_idx].push_back(atof(value.c_str()));
                }
                group_idx++;
            }
            ...
            std::stringstream group_list(cslist);
            std::string group_values;
            probs = std::vector<std::vector<double>>(K); // Initialize with K empty vectors
            int group_idx = 0;

            /*
            std::string probs_str = cslist;
            std::cout << "Raw probs string: [" << probs_str << "]" << std::endl;
            int at_count = std::count(probs_str.begin(), probs_str.end(), '@');
            std::cout << "Number of @ characters found: " << at_count << std::endl;
            */

            while (getline(group_list, group_values, '@')) {
                if (group_idx >= K) {
                    std::cout << "FATAL: too many groups in probs list (expected " << K << ")" << std::endl;
                    exit(EXIT_FAILURE);
                }
        ...
        /*else if (!strcmp(argv[i], "--predict")) {
            ...
            if (i == argc - 1) fail_if_last(argv, i);
            if (atoi(argv[i + 1]) < 1) {
                std::cout << "FATAL: number of groups K must be positive" << std::endl;
                exit(EXIT_FAILURE);
            }
