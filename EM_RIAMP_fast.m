function [xhat_all, corr, sig2_all] = EM_RIAMP_fast(n_iter, distr, y, A, true_x)
    ...
    %mu_all(1)
    sig2_all(1) = max( sum( x_all(1,:) .^2 ) / d - mu_all(1) ^ 2 * (distr.eta' * distr.probs), 1e-9 ); %since we know that Xt = mu* X + Wt, and X and Wt are indep
    ...
    for i_iter = 2:n_iter
        ...
        %updating GM parameters
        [distr]  = update_Mixture( mu_all(1:(i_iter - 1)), omega_mat(1:(i_iter-1), 1:(i_iter-1)), distr, xhat_all(1:(i_iter-1),:) );
        'probs:'
        distr.probs
        %distr_up.probs
        'eta:'
...
function [distr_up]  = update_Mixture(mu, Sigma, distr, x)
    %dim(mu) = m x 1
    %dim(Sigma) = m x m
    %dim(x) = m x d
    ...
    varsw0 = vars(2:end);
    iSigma = inv(Sigma);
    lambda = 1 - probs(1) % we assume that eta(1) = 0
    probsw0 = probsw0 / lambda; 

    %updating lambda
    pis = sum( exp ( ((mu' * iSigma * x) .^ 2) .* varsw0 / 2 ./ (mu'*iSigma*mu * varsw0 + 1) ) .* probsw0 ./ sqrt( abs( varsw0 .* ( mu' * iSigma * mu ) + 1) ), 1);
    pis = 1 ./ ( 1 + (1 - lambda) / lambda ./ ( pis ) );
    %dim(pis) = (1 x d)
    lambda = mean(pis);
    ...
    %updating variances
    phis = max(varsw0);
    expe = exp ( (x'*iSigma*mu).^2 * (varsw0' - phis) / 2 ./ ( (mu' * iSigma * mu) * varsw0' + 1 ) / ( (mu' * iSigma * mu) * phis + 1 ) );
    %dim(exp) = d x 1
    Br = sum( probsw0' .* (  ( (varsw0').^2 ) .* (x'* iSigma * mu).^2 + ( mu' * iSigma * mu * varsw0' + 1) .* varsw0' ) ./ (mu' * iSigma * mu * varsw0' + 1).^(5/2) .* expe, 1 );
    Naz = sum( probsw0' ./ sqrt( mu' * iSigma * mu * varsw0' + 1) .* expe, 1); 
    varsw0 = (Br ./ Naz)';
    %dim(varsw0) = (L-1) x 1
    %}
    ...
    beta = probsw0' ./ sqrt( abs(mu' * iSigma * mu * varsw0' + 1) ) .* expe;
    %dim(beta) = d * (L-1)
    beta = beta ./ sum(beta, 2);
    probsw0 = ( sum( pis' .* beta, 1 ) / sum( pis ) )';

    distr_up = distr;
    distr_up.eta = [0; varsw0];
    distr_up.probs = [1-lambda; lambda * probsw0];
