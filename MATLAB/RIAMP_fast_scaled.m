function [xhat_all, corr, sig2_all] = RIAMP_fast_scaled(n_iter, distr, y, I1n, I1p, I2n, I2p, true_x, n, d, A)
    ...
    %A = sparse(I1n, I1p, ones( length(I1n), 1 ) );
    %A( I2n, I2p ) = 2;
    %means = full(mean(A));
    rng(1);
    means = zeros(d,1);
    for i0 = 1:length(I1p)
        means( I1p(i0) ) = means( I1p(i0) ) + 1;
    end
    for i0 = 1:length(I2p)
        means( I2p(i0) ) = means( I2p(i0) ) + 2;
    end
    means = means / n;

    stds = zeros(d,1);
    for i0 = 1:length(I1n)
        %stds( I1p(i0) ) = stds( I1p(i0) ) + (1 - means( I1p(i0) ))^2;
        stds( I1p(i0) ) = stds( I1p(i0) ) + 1;
    end
    for i0 = 1:length(I2n)
        %stds( I2p(i0) ) = stds( I2p(i0) ) + (2 - means( I2p(i0) ))^2;
        stds( I2p(i0) ) = stds( I2p(i0) ) + 4;
    end
    stds = sqrt(stds / (n - 1) - n / (n - 1) * means.^2);
    ...
    xvec = zeros(d,1);
    xvec(1) = 1;
    s1 = Ax(I1n, I1p, I2n, I2p, means, stds, xvec, n, d);
    ...
    %clear A;
    delta = n / d;
    freecum = compute_cum(I1n, I1p, I2n, I2p, 2*(n_iter + 1), n, d, means, stds) %to be upscaled in the future
    freecum(1)
    freecum(2)
    'cc'
    ...
    x_all(1,:)  = ATx(I1n, I1p, I2n, I2p, means, stds, zhat_all(1,:)', n, d);
    mu_all(1) = 0;
    for i_iter = 1:2
        mattmp = phitmp * s_mat^(i_iter-1);
        mu_all(1) = mu_all(1) + delta * freecum(i_iter) * mattmp(2,1); % this is calculation of mu_1, equiv to (3.8)
    end
    %mu_all(1)
    ...
    tmp1 = Ax(I1n, I1p, I2n, I2p, means, stds, xhat_all(1,:)', n, d);
    z_all(1,:)  = Ax(I1n, I1p, I2n, I2p, means, stds, xhat_all(1,:)', n, d) - freecum(1) * psi_mat(2,2) * zhat_all(1,:)';
    if nargin >= 11
        %tmp = A*xhat_all(1,:)';
        %'tmp - tmp1:'
        %norm(tmp - tmp1)
    ...
    for i_iter = 2:n_iter
        ...
        x_all(i_iter, :) = ATx(I1n, I1p, I2n, I2p, means, stds, zhat_all(i_iter, :)', n, d) - ( beta * xhat_all(1:(i_iter-1), :) )';
        ...
        z_all(i_iter, :) = Ax(I1n, I1p, I2n, I2p, means, stds, xhat_all(i_iter, :)', n, d) - ( alpha * zhat_all(1:i_iter, :) )';
...
function [cumulants] = compute_cum(I1n, I1p, I2n, I2p, numbC, N, M, means, stds)
    ...
    s = normrnd( 0, 1, M, L );
    %s = A*s;
    s = Ax(I1n, I1p, I2n, I2p, means, stds, s, N, M);

    for k = 1 : numbC
        %mk1 = [ mean( lambdas .^ k ), mk1 ]
        %AAT = AAT * cov_mat;
        %mk = [ trace(AAT) / N , mk]
        mk = [ mean( sum(s .^ 2) / N ), mk];
        if mod(k,2) == 1
            %s = A' * s;
            s = ATx(I1n, I1p, I2n, I2p, means, stds, s, N, M);
        else
            %s = A * s;
            s = Ax(I1n, I1p, I2n, I2p, means, stds, s, N, M);
        end
...
function [val]  = ATx(I1n, I1p, I2n, I2p, means, stds, x, n, d)
    % dim(A) = n x d
    % dim(means) = d x 1
    %tic()
    val = zeros(d,1);
    val = val - means ./ stds * sum(x);
    for i0 = 1:length(I1n)
        val( I1p(i0) ) = val( I1p(i0) ) + x( I1n(i0) ) / stds( I1p(i0) );
    end
    for i1 = 1:length(I2n)
        val( I2p(i1) ) = val( I2p(i1) ) + 2 / sqrt(n) * x( I2n(i1) ) / stds( I2p(i1) );
    end
    %time = toc()
end


function [val]  = Ax(I1n, I1p, I2n, I2p, means, stds, x, n, d)
    % dim(A) = n x d
    % dim(means) = d x 1
    val = zeros(n,1);
    cor = sum(x .* means ./ stds);
    for i0 = 1:length(I1n)
        val( I1n(i0) ) = val( I1n(i0) ) + x( I1p(i0) ) / stds( I1p(i0) );
    end
    for i0 = 1:length(I2n)
        val( I2n(i0) ) = val( I2n(i0) ) + 2.0 * x( I2p(i0) ) / stds( I2p(i0) );
    end   
    val = val - cor;    
end